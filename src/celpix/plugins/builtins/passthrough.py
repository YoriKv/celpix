...
What the optional Reshape and Compression stages select when there is nothing to
undo or unpack: the bytes flow through unchanged, so each stage stays first-class
in the pipeline rather than being conditionally skipped. Every real scheme — a
split-ROM join, Konami RLE, the SNES LZ family — is an ordinary drop-in plugin at
the same stage (``docs/design/overview.md`` §2).
"""
