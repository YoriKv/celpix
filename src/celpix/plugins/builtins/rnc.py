...
Encrypted ("keyed") streams are refused: the key is not in the data. The unpacked
size is capped at :data:`MAX_UNPACKED` as a memory guard, since the header's
32-bit field is the one thing a corrupt stream can make arbitrarily large.
...
# Memory guard, not a format limit: far past any cartridge-era structure, while a
# corrupt 32-bit size field could otherwise ask for gigabytes.
MAX_UNPACKED = 16 << 20
...
def decompress(
    ...
    if target > MAX_UNPACKED:
        raise _fail(f"declared size {target:,} is past the {MAX_UNPACKED:,} cap")
...
class Rnc1Compression(_RncBase):
    _method = METHOD_1
    info = PluginInfo(
        id="compression.rnc1",
...
class Rnc2Compression(_RncBase):
    _method = METHOD_2
    info = PluginInfo(
        id="compression.rnc2",
